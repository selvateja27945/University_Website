...
<body>
	...
	<div align="center">
		<h1>Explore Clubs</h1>
		<form:form name="volunteerServices" action="/addvolunteers" method="post" modelAttribute="servicesdetails">
			<table>
			<tr>
				<td><form:label path="serviceId">Service Id: </form:label></td>
				<td><form:input path="serviceId"></form:input></td>
			</tr>
			<tr>
				<td><form:label path="enrolnumber">User Id: </form:label></td>
				<td><form:input path="enrolnumber" value="${userSession.enrolnumber }" readonly="true"></form:input></td>
			</tr>
			</table>
	...
	<div align="center" class="club">
		<c:if test="${volunteers == true }"><h3>*Successfully Added</h3></c:if>
		<c:if test="${volunteers == false }"><h3>*Cannot Volunteer</h3></c:if>
	</div>
	<div align="center" class="club">
	<c:if test="${allservices.size()==0 }"><h2>No Services Available</h2></c:if>
	<c:if test="${allservices.size() > 0 }">
			...
			<table>
				...
				<c:forEach items="${allservices }" var="list">
					<tr>
						<td>${list.serviceId }</td>
						<td>${list.serviceName }</td>
						<td>${list.description }</td>
						<td>${list.date }</td>
						<td>${list.volunteers }</td>
					</tr>
				</c:forEach>
			</table>
	</c:if>
